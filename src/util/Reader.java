...
//        if (values.length != Transaction.class.getDeclaredFields().length) return null;
        ...
        if(values[4].equals(Constants.PAYMENT_METHOD_CARD) ||
                values[4].equals(Constants.PAYMENT_METHOD_TRANSFER)){
    ...
    private static BinMapping processBinMapping(String line) {
        ...
        if(values[3].equals(Constants.DEBIT_CARD) ||
                values[3].equals(Constants.CREDIT_CARD)){
